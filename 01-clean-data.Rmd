...
print(fec_dec_1993_2023_names)

```


### 3.2.7. FEC & DEC - ANEEL: Join datasets "samp_1991_2023_names" and "fec_dec_1993_2023_names"
```{r}
# Select only the required columns from fec_dec_1993_2023_names and handle duplicates
fec_dec_selected <- fec_dec_1993_2023_names %>%
...
```{r}
# Select the required columns and handle duplicates by calculating the mean
df_financial_1996_2023_selected <- df_financial_1996_2023_names %>%
  select(CNPJ, Year, netprofit_margin, debtequity_ratio) %>%
  group_by(CNPJ, Year) %>%
  summarise(
    netprofit_margin = mean(netprofit_margin, na.rm = TRUE),  # Calculate average of netprofit_margin
    debtequity_ratio = mean(debtequity_ratio, na.rm = TRUE),  # Calculate average of debtequity_ratio
