...
ipca_monthly<-ipca_monthly%>%mutate(deflator93=indice/100)%>%
  mutate(deflator122020=deflator93[ano==2023 & mes==12]/deflator93)
...
df_1991_2023 <- samp_1991_2023_names %>%
  mutate(Tariff = Revenue / ConsumptionEnergy)


#Impute monthly values by taking the average consumption of that year
# create annual data set
df_annual <- df_1991_2023 %>%
  group_by(Firm_After, Year, Class, Type, Privatization) %>%
  summarize(
    revenue = ifelse(all(is.na(Revenue)), NA_real_, sum(Revenue, na.rm = TRUE)),
    revenue_tax = ifelse(all(is.na(RevenueTax)), NA_real_, sum(RevenueTax, na.rm = TRUE)),
    ...
    tax = ifelse(all(is.na(Tax)), NA_real_, sum(Tax, na.rm = TRUE))
  )
...
df_annual <- df_annual %>%
  ...
  mutate(
    tariff_growth = if_else(
      ...
      (Tariff / Tariff[which(t_privat == 0)][1]) * 100,  # Use the first instance where t_privat == 0
...
df_summary <- df_annual %>%filter(Year>1993)%>%
  group_by(Class, t_privat) %>%
  summarize(
...
ggplot(df_summary, aes(x = t_privat, y = mean_tariff_growth)) +
  ...
  #geom_point(aes(x = t_privat, y = median_tariff_growth), color = "red", shape = 16) +  # Median points
  geom_vline(xintercept = 0, color = "black", linetype = "dashed") +  # Vertical line at t_privat = 0
  scale_x_continuous(limits = c(-10, 10)) +  # X-axis limits
  scale_y_continuous(limits = c(0, 400)) +  # Y-axis limits
  labs(
    title = "Tariff Growth by Class and t_privat",
    x = "t_privat",
    y = "Tariff Growth (%)"
  ) +
  facet_wrap(~Class, scales = "free_y") +  # Free y-scales for each facet
