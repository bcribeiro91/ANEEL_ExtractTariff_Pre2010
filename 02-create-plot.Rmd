...
graph_ownership_br <- df_all_firms %>%  # Group by owner_type and count the number of firms in each category
                group_by(owner_type) %>%
...
png("C:/Users/BEATRIZCO/OneDrive - Inter-American Development Bank Group/Documents/INE-WSA/Projects/Privatization Infrastructure/figures/firms_ownership_br.png", units="in", width=9, height=7, res=300)

print(graph_ownership_br)
...
graph_time_privatization <- ggplot(df_priv_count, aes(x = Privatization, y = count_firms)) +
...
png("C:/Users/BEATRIZCO/OneDrive - Inter-American Development Bank Group/Documents/INE-WSA/Projects/Privatization Infrastructure/figures/time_privatization.png", units="in", width=9, height=7, res=300)

print(graph_time_privatization)
...
df_all_firms_long <- df_all_firms %>%
  pivot_longer(cols = c(SAIDI, SAIFI, netprofit_margin, debtequity_ratio, revenue, consumers, consumption, B1_RESIDENCIAL, B2_RURAL, B3_DEMAIS, B4a_REDE_DE_DISTRIBUICAO),
               names_to = "variable", 
               values_to = "value")
...
# Reshape the data to long format
df_all_firms_long <- df_all_firms %>%
  pivot_longer(cols = c(consumers, consumption),
               names_to = "variable", 
               values_to = "value")
...
# Selecting only some variables to plot a graph
# Calculate the average of each variable for each year
df_all_firms_avg <- df_all_firms_long %>%
  group_by(year_before_after, variable) %>%
  summarise(avg_value = mean(value, na.rm = TRUE), .groups = "drop")
...
# Plot with geom_line for the average values
graphs_coverage <- ggplot(df_all_firms_avg, aes(x = year_before_after, y = avg_value, color = variable, group = variable)) +
  geom_line() +  # Adds lines for each variable
  facet_wrap(~ variable, scales = "free_y", nrow = 1) +  # Creates separate plots for each variable
  labs(
    title = "Average Time Series of Different Variables",
    x = "Year (Before/After)",
    y = "Average Value"
...
# png("C:/Users/BEATRIZCO/OneDrive - Inter-American Development Bank Group/Documents/INE-WSA/Projects/Privatization Infrastructure/figures/coverage_before_after.png", units="in", width=10, height=7, res=300)
# 
print(graphs_coverage)
# 
# dev.off()
...
corrplot(corr = cor(df_all_firms[, c(4:11, 24:28)], use="pairwise.complete.obs"),
         addCoef.col = "white",
         number.cex = 0.8,
         number.digits = 1,
         diag = FALSE,
...
GGally::ggpairs(df_all_firms[, c(4:11, 20, 24:28)],
...
#transform "privatization_year" into numeric to run the code
df_all_firms <- df_all_firms %>%
...
# Staggered DiD (CS2021)
```{r}

# Step 1: Identify first year of treatment (gname)
df_all_firms <- df_all_firms |>
  dplyr::group_by(idname) |>
  dplyr::mutate(group = ifelse(any(privatization_year == 1),
                                min(Year[privatization_year == 1]),  # First year treated
                                Inf)) |>  # Inf = never treated
  dplyr::ungroup()


group_effectH <- att_gt(
  yname = "B2_RURAL",
  gname = "group",              # First year treated (constructed above)
  idname = "idname",            # Unit identifier
  tname = "Year",               # Time variable
  xformla = ~ 1,                # No covariates; use ~ x1 + x2 if needed
  data = df_all_firms,
  est_method = "reg",           # or "dr" for doubly robust
  allow_unbalanced_panel = TRUE,
  bstrap = TRUE,
  clustervars = "idname"        # Recommended
)
summary(group_effectH)


# Dynamic effects (event study)
es <- aggte(group_effectH, type = "dynamic", min_e = -5, max_e = 10, na.rm = TRUE)
summary(es)
ggdid(es, title = "Event Study Plot")

# Overall ATT
overall <- aggte(group_effectH, type = "group", na.rm = TRUE)
summary(overall)
...
# Loop function - Two way fixed effects Callaway and santanna
```{r}
#Function
library(did)
library(ggplot2)

run_did_analysis <- function(yvar, data, gname = "group", idname = "idname", tname = "Year",
                             xformla = ~1, method = "reg", min_e = -5, max_e = 10) {
  
  message(paste0("Estimating ATT for outcome: ", yvar))
  ...
  att <- att_gt(
    ...
    bstrap = TRUE,
    clustervars = idname,
    control_group = "notyettreated"   # ✅ here is the key change

  )
  
  # Dynamic effects
  es <- aggte(att, type = "dynamic", min_e = min_e, max_e = max_e, na.rm = TRUE)
  
  # Return results
  list(
    att_gt = att,
    es = es,
    plot = ggdid(es, title = paste("Event Study:", yvar))
  )
}

colnames(df_all_firms)

# Run for multiple outcomes
outcomes <- c("SAIDI", "SAIFI", 
              "log_SAIDI", "log_SAIFI", 
              "B1_RESIDENCIAL", "B2_RURAL", "B3_DEMAIS", 
              "B4a_REDE_DE_DISTRIBUICAO", 
              "log_revenue", "log_consumption", "log_consumers", "log_tax")

did_results <- lapply(outcomes, function(var) {
  run_did_analysis(yvar = var, data = df_all_firms)
})
names(did_results) <- outcomes


# Define your output directory
output_dir <- "C:/Users/BEATRIZCO/OneDrive - Inter-American Development Bank Group/Documents/INE-WSA/Projects/Privatization Infrastructure/Results"

# Ensure the folder exists (optional safeguard)
dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

# Save each plot as PNG to the specified directory
for (name in names(did_results)) {
  plot_obj <- did_results[[name]]$plot
  
  ggsave(
    filename = file.path(output_dir, paste0(name, "_event_study.png")),
    plot = plot_obj,
...
untreated<-df_all_firms%>%filter(is.infinite(group))
```
...
#SAIDI
reg_saidi_twoway <- plm(SAIDI ~ privatization_year, data = df_all_firms, model = "within",
                   index = c("Firm_Final", "Year"))
summary(reg_saidi_twoway)


#SAIFI
reg_saifi_twoway <- plm(SAIFI ~ privatization_year, data = df_all_firms, model = "within",
                   index = c("Firm_Final", "Year"))
summary(reg_saifi_twoway)


#PROFIT
reg_profit_twoway <- plm(netprofit_margin ~ privatization_year, data = df_all_firms, model = "within",
                   index = c("Firm_Final", "Year"))
summary(reg_profit_twoway)


#DEBIT
reg_debit_twoway <- plm(debtequity_ratio ~ privatization_year, data = df_all_firms, model = "within",
                   index = c("Firm_Final", "Year"))
summary(reg_debit_twoway)

#consumers
reg_consumers_twoway <- plm(consumers ~ privatization_year, data = df_all_firms, model = "within",
                   index = c("Firm_Final", "Year"))
summary(reg_consumers_twoway)


#consumption
reg_consumption_twoway <- plm(consumption ~ privatization_year, data = df_all_firms, model = "within",
                   index = c("Firm_Final", "Year"))
summary(reg_consumption_twoway)

#revenue
reg_revenue_twoway <- plm(revenue ~ privatization_year, data = df_all_firms, model = "within",
                   index = c("Firm_Final", "Year"))
summary(reg_revenue_twoway)
