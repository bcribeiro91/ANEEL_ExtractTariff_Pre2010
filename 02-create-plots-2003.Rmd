...
df_all_firms <- df_all_firms %>%
  mutate(across(c(revenue, revenue_tax, consumption, consumers, tax, B1_RESIDENCIAL, B2_RURAL, B3_DEMAIS, B4a_REDE_DE_DISTRIBUICAO), 
...
reg_TEC <- plm(SAIFI ~ privatization_year + log_consumers + debtequity_ratio + B1_RESIDENCIAL, data = df_all_firms, model = "pooling", index = c("Firm_Final", "Year"))
  summary(reg_TEC)
